...
<div class="container">
    <h1>Book Category Info</h1>
    <form action="catagoryprocess.php?update=true" method="post">
        <?php
        // Initialize $category_id variable to avoid "Undefined array key" warning
        $category_id = "";
        ...
        // Fetch the category information based on the provided category_id
        $sql = "SELECT category_id, category_Name, date_modified FROM bookcategory WHERE category_id = '$category_id'";
        $result = $conn->query($sql);    

        // Display category information if found
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo '<div class="form-group">';
                echo '<label for="category_id">Category ID</label>';
                echo '<input type="text" id="category_id" name="category_id" value="' . $row['category_id'] . '" required>';
                echo '</div>';

                echo '<div class="form-group">';
                echo '<label for="category_Name">Category Name</label>';
                echo '<input type="text" id="category_Name" name="category_Name" value="' . $row['category_Name'] . '" required>';
                echo '</div>';

                echo '<div class="form-group">';
                echo '<label for="date_modified">Data Modified</label>';
                echo '<input type="date_modified" id="date_modified" name="date_modified" value="' . $row['date_modified'] . '" readonly>';
                echo '</div>';
            ...
            echo "No categories found with the provided ID.";
        }
        ?>
        <script>
